// frontend/src/app/dynamic_page/new/page.tsx
'use client';
...
import { useEditorStore, Layer, SavedState } from '@/store/dynamicEditorStore';
...
export default function EdgeEditorPage() {
    const { scenes, addScene, addLayer, reset, loadState, overwriteConfirm, clearOverwriteConfirm, moveScene, fetchCanvasResolutions } = useEditorStore();
    ...
    const processAndAddLayer = (file: File, sceneId: string, regionId: string) => {
        ...
        reader.onload = (e) => {
            ...
            addLayer(sceneId, regionId, newLayer);
        };
    ...
    const handleContentTypeSelect = (contentType: Layer['type']) => {
        ...
        if (contentType === 'text') {
            ...
            if (text) {
                ...
                addLayer(uploadInfo.sceneId, uploadInfo.regionId, newLayer);
            }
        } else {
    ...
    const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file && uploadInfo) {
            ...
            processAndAddLayer(file, sceneId, regionId);
        }
    ...
    const handleSave = async () => {
        ...
        try {
          if (currentProjectId) {
            ...
            router.push(`/dynamic_page/new?id=${response.data.id}`);
          }
        } catch (error) {
            if (axios.isAxiosError(error) && error.response?.status !== 401) {
    ...
    const handleLoadProject = (projectId: number) => {
        router.push(`/dynamic_page/new?id=${projectId}`);
        setIsLoadModalOpen(false);
    };

    const handleExport = () => {
        // This function needs to be updated to handle layers
        alert("내보내기 기능은 레이어 구조에 맞게 업데이트해야 합니다.");
    };
